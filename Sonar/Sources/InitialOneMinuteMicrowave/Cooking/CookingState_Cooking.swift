...
public final class CookingState_Cooking: CookingState {
    ...
    public init(
        _ name: String,
        transitions: [Transition<CookingState_Cooking, CookingState>] = [],
        gateway: FSMGateway,
        clock: Timer
    ) {
    ...
    public override final func clone() -> CookingState_Cooking {
        let transitions: [Transition<CookingState_Cooking, CookingState>] = self.transitions.map { $0.cast(to: CookingState_Cooking.self) }
        let state = CookingState_Cooking(
            self.name,
...
extension CookingState_Cooking: CustomStringConvertible {
