...
final class LightFiniteStateMachine: MachineProtocol, CustomStringConvertible {
    ...
    var light = InMemoryVariable<Bool>(name: "light", initialValue: false)
    ...
    var externalVariables: [AnySnapshotController] {
        get {
            [AnySnapshotController(buttonPushed), AnySnapshotController(doorOpen), AnySnapshotController(light)]
        } set {
    ...
    lazy var initialState: MicrowaveState = {
        CallbackMicrowaveState(
            ...
            snapshotSensors: [buttonPushed.name, doorOpen.name, timeLeft.name, light.name],
            snapshotActuators: [buttonPushed.name, doorOpen.name, light.name],
    ...
    func clone() -> LightFiniteStateMachine {
        let fsm = LightFiniteStateMachine(name: name, buttonPushed: buttonPushed.clone(), doorOpen: doorOpen.clone(), timeLeft: timeLeft.clone(), light: light)
        fsm.name = name
    ...
    init(name: String, buttonPushed: InMemoryVariable<Bool>, doorOpen: InMemoryVariable<Bool>, timeLeft: InMemoryVariable<Bool>, light: InMemoryVariable<Bool>) {
        self.name = name
